...
def leftmost(arr:[int],target:int):
    left=0
    right=len(arr)-1
    while left<right-1:
        mid=(left+right)//2
    ...
    if arr[right]==target:
        return right
    return -1
def rightmost(arr:[int],target:int):
    left = 0
    right = len(arr) - 1
    while left < right - 1:
        mid = (left + right) // 2
...
def slowest(orders: [[int]]) -> int:
    timelist = [0 for i in range(20)]
    starttime = 0
    for j in orders:
        broker_id = j[0]
        time = j[1] - starttime
        starttime = j[1]
        timelist[broker_id] = max(timelist[broker_id], time)
    maxtime = max(timelist)
    return timelist.index(maxtime)
