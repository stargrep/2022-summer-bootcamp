...
def update_pricing_data(symbol: str) -> int:
    ...
    # TODO implement logic to update `stock_price` table use database_service methods.
    ...
    try:
        ...
        if res is None or len(res) == 0:
            ...
            latestTime = execute_read(f"SELECT trade_date FROM stock_price WHERE symbol = '{symbol}' ORDER BY trade_date DESC LIMIT 1")
            latest_Date_Time = datetime.strptime(latestTime[0][0], '%Y-%m-%d %H:%M:%S')
            count = 0
            for i, row in df.iterrows():
                trade_date = row['Trade_date']
